...
class Report:
    ...
    def __init__(self, results, test_set, metrics):
        ...
        if isinstance(self._test_set, pd.DataFrame):
            self._test_set = DataFrame2XYCDataset(self._test_set)
    ...
    def _parse_results(self, field):
        ...
        for model_result in self._results.values():
            result.append(np.array([score[field] for score in model_result[
                          'result']['metric_cv_results']]).mean())
    ...
    def _print_differences_dataframe(self, metric, diffs):
        ...
        print(pd.DataFrame(formatted, columns=[
              'tuned'], index=self._results.keys()))
    ...
    def _validate_metric(self, metric):
        ...
        if not isinstance(metric, Metric):
            raise ValueError(
                "metric must be a Metric class, found {}".format(type(metric)))
    ...
    def _plot_metric_results(self, metric):
        ...
        for i, (model_name, model_result) in enumerate(self._results.items()):
            ...
            cv_results = np.array([score[metric.name] for score in model_result[
                                  'result']['metric_cv_results']])
            errors = np.fabs(
                np.array([cv_results.min(), cv_results.max()]).reshape(2, -1) - values)
            pyplot.errorbar(i, values, yerr=errors, fmt='.',
                            markersize=25, capsize=10, capthick=2, barsabove=True)
            xticks.append(model_name + ' tuned\n%.5f' %
                          self._metric_results[metric][i])
    ...
    def _plot_heatmap(self, metric):
        ...
        for model_name, model_class in self._results.items():
            model = model_class['model_space'].model_class(
                params=model_class['result']['params'])
